...
public class MovieActorsDaoImplementation implements MovieActorsDao{
    ...
    @Override
    public List<Person> getMovieActors(Integer movieId) throws SQLException
    {
        List<Person> persons = new ArrayList<>();
        String sql = "SELECT * FROM persons WHERE id IN (SELECT person_id FROM movie_actors WHERE movie_id = ?)";
        PreparedStatement psmt = conn.prepareStatement(sql);
        ResultSet result;
